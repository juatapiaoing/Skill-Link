...
const Perfil = () => {
  ...
  useEffect(() => {
    ...
    // Cargar datos del primer trabajador (simulando sesión)
    const personas = loadFromLocalStorage<Persona[]>('personas', []);
    const trabajador = personas.find(p => p.tipo === "Trabajador");
    
    if (trabajador) {
      setPersona(trabajador);
      
      const todosServicios = loadFromLocalStorage<Servicio[]>('servicios', []);
      setServicios(todosServicios.filter(s => s.trabajadorId === trabajador.id));
      
      const todasCertificaciones = loadFromLocalStorage<Certificacion[]>('certificaciones', []);
      setCertificaciones(todasCertificaciones.filter(c => c.trabajadorId === trabajador.id));
      
      const todosCV = loadFromLocalStorage<Curriculum[]>('curriculums', []);
      setExperiencias(todosCV.filter(cv => cv.trabajadorId === trabajador.id));
    }
  }, []);
